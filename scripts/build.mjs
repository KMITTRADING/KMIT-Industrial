...
import {render,origin,googleTag} from '../src/render.mjs';
...
const bundle = await build({entryPoints:['src/client.js'],bundle:true,splitting:true,format:'esm',outdir:'dist/assets',minify:true,target:['es2022'],entryNames:'[name]',chunkNames:'[name]-[hash]',metafile:true});
await fs.writeFile('docs/bundle-report.json',JSON.stringify(bundle.metafile,null,2));
const assetDirectory = path.resolve('dist/assets');
const outputs = new Set(Object.keys(bundle.metafile.outputs).map(file=>path.resolve(file)));
for (const entry of await fs.readdir(assetDirectory,{withFileTypes:true})) {
  const file = path.resolve(assetDirectory,entry.name);
  if (entry.isFile() && path.dirname(file) === assetDirectory && /\.js$/.test(entry.name) && !outputs.has(file)) await fs.unlink(file);
}
...
await fs.writeFile('dist/index.html',`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="refresh" content="0;url=/ar/">${googleTag}<title>KMIT | مواد وحلول صناعية</title><link rel="canonical" href="${origin}/ar/"><link rel="alternate" hreflang="ar" href="${origin}/ar/"><link rel="alternate" hreflang="en" href="${origin}/en/"></head><body><a href="/ar/">العربية</a> | <a href="/en/">English</a></body></html>`);
await fs.writeFile('dist/404.html',`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex">${googleTag}<title>الصفحة غير موجودة | Page not found · KMIT</title><link rel="stylesheet" href="/styles.css"></head><body><main class="section"><a class="brand" href="/ar/"><img src="/logo.svg" width="1920" height="649" alt="KMIT Industrial Solutions"></a><p class="eyebrow">404</p><h1>الصفحة غير موجودة</h1><p lang="en" dir="ltr">The page you’re looking for could not be found.</p><div class="hero-actions"><a class="button" href="/ar/">العودة للرئيسية</a><a class="text-link" href="/en/" lang="en">English homepage</a></div></main></body></html>`);
...
await fs.writeFile('dist/_headers',`/*\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n  Permissions-Policy: camera=(), microphone=(), geolocation=()\n  Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline' https://www.googletagmanager.com; style-src 'self'; img-src 'self' data: https://www.google-analytics.com; font-src 'self'; connect-src 'self' https://www.google-analytics.com https://analytics.google.com https://region1.google-analytics.com; object-src 'none'; base-uri 'self'; frame-ancestors 'none'\n/assets/*\n  Cache-Control: public, max-age=3600\n/images/*\n  Cache-Control: public, max-age=86400\n/fonts/*\n  Cache-Control: public, max-age=31536000, immutable\n`);
